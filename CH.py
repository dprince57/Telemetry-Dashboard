...
def init_car(car_id: int, spec_path: str, aggression: float = 0.7, is_player: bool = False) -> CarState:
    ...
    car = CarState(car_id=str(car_id), carSpec=spec, aggression=aggression)
...
def send_telemetry_to_pit_wall(car: CarState, sim_t: float, track: Optional[TrackHandler.Track] = None) -> None:
    ...
    IMPORTANT:
    The dashboard expects 'track_position' and 'speed_mps' keys.
    We also include legacy keys ('position', 'velocity_mps') for backwards compatibility.
    """
    ...
    if track is not None:
        try:
            lap_len = float(track.lap_length_meter)
        except Exception:
            lap_len = None
    ...
    telemetry = {
        ...
        "tire_RR_temp_C": car.tire_RR.temp_C,
        "tire_FL_wear": car.tire_FL.wear,
        "tire_FR_wear": car.tire_FR.wear,
        "tire_RL_wear": car.tire_RL.wear,
...
def run_sim(car: CarState, dt: float, track: TrackHandler.Track, sim_t: float, 
    ...
    # Check for pit commands from pit wall
